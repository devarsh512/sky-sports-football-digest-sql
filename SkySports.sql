...
select
	team,
	(wins / matches_played)* 100 as wins_percent
from
	group_stage_team_stats;

#7. Write an SQL query to show which team has maximum goals_scored and their count(how much)
select max(goals_scored) from group_stage_team_stats; #9
...
/*8. Write an SQL query to show percent of draws with 
 *   respect to matches_played round of to 2 digits by each team*/
select
	team,
	round(((draws / matches_played)* 100), 2) as draws_percent
from
	group_stage_team_stats;

#9. Write an SQL query to show which team has minimum goals_scored and their count
select min(goals_scored) from group_stage_team_stats; #1
...
/*10. Write an SQL query to show percent of losses 
 *    with respect to matches_played by each team in ascending order 
 *    by losses and name the resulting column as losses_percent. */
select
	team,
	round(100 *(losses / matches_played), 2) as losses_percent
from
	group_stage_team_stats
order by
	losses_percent;
...
from
	group_stage_team_stats
where
	exp_goal_difference between -0.5 and 0.5;
...
from
	group_stage_team_stats
order by
	exp_goal_difference_per_90 desc;
...
select
	avg(possession) as Avg_Possession_Of_Teams
from
	overall_wc_stats;
...
select
	...
	shots_on_target_pct
from
	overall_wc_stats ows
where
	shots_on_target_pct < 30
order by
	ows.shots_on_target_pct;
...
from
	overall_wc_stats
order by
	average_shot_distance desc;

/*29. Write an SQL query to show team, errors, touches for 
 *    which errors is 0 and touches is less than 1500 */
select
	team,
	errors,
	touches
...
select
	`group`,
	min(exp_goal_conceded)
from
	group_stage_team_stats
group by
	`group`
order by
	min(exp_goal_conceded) desc;    
...
select
	`group`,
	avg(exp_goal_difference_per_90)
from
	group_stage_team_stats
group by
	`group`
...
#41. Write an SQL query to show which team has maximum players_used.(#16...)
 select
	team,
	players_used
from
...
select
	...
	avg(os.avg_age)
from
	group_stage_team_stats gs
inner join overall_wc_stats os
on
	gs.team = os.team
group by
	gs.`group`
...
/*46. Write an SQL query to show sum of fouls for each group and arrange it 
*     in ascending order by fouls. */
select * from group_stage_team_stats;
select * from overall_wc_stats;

select
	gs.`group`,
	sum(os.fouls) as sum_of_fouls
from
	group_stage_team_stats gs
inner join overall_wc_stats os
on
...
/*47. Write an SQL query to show total number of games for each group and arrange it
 *    in descending order by games */
select
	gs.`group`,
	sum(os.games) as Total_games
from
	group_stage_team_stats gs
inner join overall_wc_stats os
on
...
select
	gs.`group`,
	sum(os.players_used) as Total_Players
from
	group_stage_team_stats gs
inner join overall_wc_stats os
on
...
/*49. Write an SQL query to show total number of offsides for each group 
 *    and arrange it in ascending order by offsides. */
select
	gs.`group`,
	sum(os.offsides) as Total_Players
from
	group_stage_team_stats gs
inner join overall_wc_stats os
on
